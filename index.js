...
function addArtist(array, myName, myYears, myGenre, myNationality, myBio) {
    let newArray = [];
    for (let i = 0; i < array.length; i++) {
     newArray.push(array[i]);
   }
...
function lotsOfArt(array) {
  let newArray = array.filter(artist => artist.paintings >= 100);
...
function artistInfo(array, myName){
  const index = array.findIndex(object => {
  return object.name === myName;
 });
...
function artistByCountry(array, myNationality){
  let newArray = array.filter(artist => artist.nationality === myNationality);
  for (let i = 0; i < newArray.length; i++) {
    newArray[i] = newArray[i].name;
  }
  return newArray;
